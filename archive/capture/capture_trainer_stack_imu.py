...
from capture import Capture, CaptureSet, IMUStackCaptureSet
...
class Trainer:
    def __init__(self, args):
        ...
        self.train_capture_set = IMUStackCaptureSet(
        ...
        self.test_capture_set = IMUStackCaptureSet(
            args.test_capture_set_dir, self.stack_size, self.device,
        )

        self.model = ResNet(self.config, 3 * self.stack_size, 42, 3).to(self.device)
    ...
    def batch_train(self):
        ...
        for i, ((imus, cameras), values) in enumerate(self.train_loader):
            outputs = self.model(cameras, imus)
    ...
    def batch_test(self):
        ...
        for i, ((imus, cameras), values) in enumerate(self.test_loader):
            outputs = self.model(cameras, imus)
            loss = self.loss(outputs, values)
            loss_meter.update(loss.item())
