...
def loss(segment, progress, track_position):
    ...
    return np.linalg.norm(
        np.array([progress, track_position]) -
        np.array([annotated_progress, annotated_track_position]),
    )
...
def course_correct(segment):
    ...
    while(running):
        ...
        l = losses[idx]
        print("LOSS {:.3f} {:.3f}".format(l, last_loss))
    ...
    # Reintegrate one last time to compute the new start conditions and store
