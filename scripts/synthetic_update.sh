...
grep TRAIN $TEMP/out.log | tail -n +10000 | cut -d' ' -f 14 | gnuplot -p -e 'set terminal png; set output "'$TEMP'/bce_loss.png"; plot "/dev/stdin";'
grep TRAIN $TEMP/out.log | tail -n +10000 | cut -d' ' -f 16 | gnuplot -p -e 'set terminal png; set output "'$TEMP'/kld_loss.png"; plot "/dev/stdin";'
grep TRAIN $TEMP/out.log | tail -n +10000 | cut -d' ' -f 18 | gnuplot -p -e 'set terminal png; set output "'$TEMP'/gan_loss.png"; plot "/dev/stdin";'
