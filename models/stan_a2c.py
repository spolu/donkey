...
def preprocess(observation):
    distance = [o.distance for o in observation]
    correction = [o.correction for o in observation]
    speed = [o.speed for o in observation]

    observation = np.concatenate(
        (np.stack(distance), np.stack(correction), np.stack(speed)),
        axis=-1,
    )
...
class A2CStorage:
    ...
    def insert(self, step, observations, hidden, action, log_probs, value, reward, mask):
...
class Model:
    ...
    def batch_train(self):
        for step in range(self.rollout_size):
            ...
            print("VALUE/STEERING/DONE/REWARD: {:.2f} {:.2f} {} {:.2f}".format(
                value.data[0][0],
                action.data[0][0],
                done[0],
                reward[0],
